...
export default function Predict() {
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      console.error(err);
      setChatMessages([...newMessages, { role: 'assistant', content: "Sorry, I am unable to connect to the CardioVision AI at this moment." }]);
    } finally {
  ...
  return (
    <div className="bg-white min-h-screen py-12 md:py-16">
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        {isChatOpen ? (
          <div className="bg-white border border-gray-200 shadow-xl rounded-lg w-80 md:w-96 flex flex-col overflow-hidden mb-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="p-4 bg-blue-600 text-white flex items-center justify-between">
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
                <h3 className="text-sm font-bold tracking-widest uppercase">CardioVision AI</h3>
              </div>
            ...
            <div className="p-4 flex flex-col gap-3 h-80 overflow-y-auto bg-slate-50">
              {chatMessages.length === 0 ? (
                <div className="text-sm text-gray-600 font-medium mt-2">
                  {result ? `Hello! Based on your ${primaryDetection?.severity} diagnosis, how can I help you today?` : "Hello! I am the CardioVision AI Assistant. You haven't uploaded an image yet. How can I help you?"}
                </div>
              ) : (
                  <div className="space-y-4">
                    {chatMessages.map((msg, idx) => (
                      <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] p-3 text-sm font-medium ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-l-lg rounded-br-lg' : 'bg-white border border-gray-200 text-gray-800 rounded-r-lg rounded-bl-lg'}`} style={{ whiteSpace: "pre-wrap" }}>
                          {msg.content}
                        </div>
                      </div>
                    ))}
                ...
                {chatLoading && (
                  <div className="flex justify-start">
                     <div className="max-w-[85%] p-3 bg-white border border-gray-200 text-gray-500 rounded-r-lg rounded-bl-lg flex items-center gap-2 text-sm font-medium">
                       <div className="w-4 h-4 border-[3px] border-gray-200 border-t-blue-600 rounded-full animate-spin"></div>
                       <span>Thinking...</span>
                     </div>
                  </div>
                )}
            ...
            {chatMessages.length === 0 && (
              <div className="px-4 pb-3 bg-slate-50 flex flex-col gap-2">
                {(result 
                  ? ["How can you help me?", "What can this app do for me?", "What does my severity mean?", "Lifestyle tips for my condition"]
                  : ["How can you help me?", "What can this app do for me?", "How do I upload an image?"]
                ).map((q, i) => (
                   <button key={i} onClick={() => handleSendMessage(q)} className="text-[11px] bg-white border border-blue-200 text-blue-700 px-3 py-2 rounded-lg hover:bg-blue-50 font-bold transition-colors text-left shadow-sm">
                     {q}
                   </button>
                ))}
              </div>
            ...
            <div className="p-3 bg-white border-t border-gray-200 flex gap-2">
              <input 
                ...
                className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500 font-medium" 
              ...
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded text-sm font-bold uppercase tracking-wide disabled:opacity-50 transition-colors flex items-center justify-center p-2"
              >
                Send
              </button>
            </div>
          ...
          <button 
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white rounded-full p-4 shadow-xl flex items-center justify-center transform transition-transform hover:scale-105 group relative z-50"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>
            <span className="absolute right-full mr-3 whitespace-nowrap bg-gray-900 text-white text-xs font-bold px-2 py-1 rounded hidden group-hover:block transition-all">
              Medical Assistant
            </span>
          </button>
        )}
